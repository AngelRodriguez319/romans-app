...
ssh -t -i .dev.pem "ubuntu@$HOST" "cd $KEY/app;sed -i \"s/run roman_numbers_reactjs/run roman_numbers_reactjs_$KEY/\" \"bin/setup.sh\""
ssh -t -i .dev.pem "ubuntu@$HOST" "cd $KEY/app;sed -i \"s/exec roman_numbers_reactjs/exec roman_numbers_reactjs_$KEY/\" \"bin/setup.sh\""
ssh -t -i .dev.pem "ubuntu@$HOST" "cd $KEY/app;sed -i \"s/roman_numbers_reactjs/roman_numbers_reactjs_$KEY/\" \"bin/docker/docker-compose.yml\""
